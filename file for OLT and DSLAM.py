...
############################# INT DES ###########################################################
    ...
    for i in match :
        print (i)
        ip.append(i[0])
    with open('IP OLT DSLAM.txt', 'w', newline = '') as fp:
        writer = csv.writer(fp, delimiter = ',')
        fp.write("\n")
        for item in ip:
            fp.write("%s\n" % item)
    ####################################### add command for bars
    # access_user = []
    # node_list = pd.read_csv('nodeip.csv', header=None)
    # entries = node_list.values
    # for i in range(len(entries)) :
    #     if("V" in entries[i][1]) :
    #         entries[i][1] = entries[i][1].replace("V","?")
    #     xxx = entries[i][1].split("?") 
    ...
    with open('OLT and DSLAM.csv', 'w', newline = '') as fp:
        writer = csv.writer(fp, delimiter = ',')
        for row in match:
            writer.writerow(row)
